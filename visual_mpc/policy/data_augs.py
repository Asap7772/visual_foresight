...
def random_crop(imgs, out_size=(64, 64)):
    """
        args:
    ...
    """
    n, c, h, w = imgs.shape
    crop_h_max = h - out_size[0] + 1
    crop_w_max = w - out_size[1] + 1
    w1 = np.random.randint(0, crop_h_max, n)
    h1 = np.random.randint(0, crop_w_max, n)
    cropped = []
    for i, (img, w11, h11) in enumerate(zip(imgs, w1, h1)):
        cropped.append(img[:, h11:h11 + out_size[0], w11:w11 + out_size[1]])
...
def random_color_jitter(imgs):
    ...
    b,c,h,w = list(imgs.shape)
    imgs = imgs.view(-1,3,h,w)
    transform_module = torchvision.transforms.ColorJitter(
                                                brightness=0.4, 
                                                contrast=0.4,
                                                saturation=0.4, 
                                                hue=0.5, 
