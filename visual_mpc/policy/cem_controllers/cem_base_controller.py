...
class CEMBaseController(Policy):
    ...
    def perform_CEM(self, state=None):
        self._logger.log('starting cem at t{}...'.format(self._t))
        self._logger.log('------------------------------------------------')

        self.K = self._hp.minimum_selection
        if self._hp.selection_frac:
            self.K = max(int(self._hp.selection_frac * self._hp.num_samples), self._hp.minimum_selection)

        if state is not None:
            last_state = state[-1]
        else: last_state = None
        actions = self._sampler.sample_initial_actions(self._t, self._hp.num_samples, last_state)
        for itr in range(self._n_iter):
            ...
            self._best_indices = scores.argsort()[:self.K]
