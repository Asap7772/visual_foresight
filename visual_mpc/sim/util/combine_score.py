...
def write_scores(conf, result_file, stat, i_traj=None):
    ...
    if any('/' in key for key in list(stat.keys())):
        prefixes = []
        for key in stat.keys():
            prefixes.append(str.split(key, '/')[0])
        prefixes = set(prefixes)

        for p in prefixes:
            path_dirs = str.split(result_file, '/')
            result_file_ = '/'.join(path_dirs[:-1]) + '/' + str.split(path_dirs[-1], '.')[0] + p + '.txt'
            write_single_score(conf, result_file_, stat, i_traj, p + '/')
    else:
        write_single_score(conf, result_file, stat, i_traj)


def write_single_score(conf, result_file, stat, i_traj=None, prefix=''):
    improvement = stat[prefix + 'improvement']
    final_dist = stat[prefix + 'final_dist']
    if 'initial_dist' in stat:
        initial_dist = stat[prefix + 'initial_dist']
